...
function Product() {
 ...
 const[isLoading,setLoading]=useState(false)
    ...
    let Loaddata= async()=>{
        setLoading(true)
        let user= await axios.get(`https://6283ca4a38279cef71dc6090.mockapi.io/teacher`)
        setData(user.data)
        setLoading(false)
    }
    ...
    try{
      ...
      if(ask){
          await axios.delete(`https://6283ca4a38279cef71dc6090.mockapi.io/teacher/${identity}`)
        Loaddata();
      }
    }catch(errors){
                         ...
                         
                          {
                              data.map((ele,index)=>{
                                return <tr>
                                  ...
                                  <td>{ele.Jobtype}</td>
                                  <td>
                                  <Link to={`/portal/teacherview/${ele.id}`} type="button" className="btn btn-primary">Read</Link>
                                  <Link to={`/portal/teacheredit/${ele.id}`} type="button" className="btn btn-warning m-2">Update</Link>
